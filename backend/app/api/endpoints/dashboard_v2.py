...
async def get_dashboard_v2(
    ...
    if accs:
        ...
        for i, (label, op_filter, _) in enumerate(cols):
            ...
            if v > 0:
                expense_breakdown.append(ExpenseSegment(
                    category=label,
                    amount=round(v, 2),
                    pct=0,  # пересчитаем после
                    op_type_filter=op_filter,
                ))

        # «Не опознано» — op_type'ы из transactions со списанием, которых нет
        # ни в _EXPENSE_BUCKETS, ни в рекламных (FBO ad), ни в financing.
        # Юзер: «данные что Ozon шлёт и не понятно куда — назови не опознано».
        from app.api.endpoints.funnel_v2 import _TX_OP_TYPE_TO_GROUP
        from app.workers.tasks.sync_financing import _FIN_OP_TO_PRODUCT
        known_ops: set[str] = set()
        for _, _, op in _EXPENSE_BUCKETS:
            if op: known_ops.add(op)
        known_ops.update(_TX_OP_TYPE_TO_GROUP.keys())
        known_ops.update(_FIN_OP_TO_PRODUCT.keys())
        # «Знаковые» revenue-операции тоже не выводим как расход
        known_ops.update({
        ...
        unmapped_rows = (await db.execute(
            select(
                ...
                func.count(Transaction.ozon_transaction_id).label("cnt"),
            ).where(
                Transaction.ozon_account_id.in_(accs),
                Transaction.time >= dt_from,
                Transaction.time < dt_to,
                Transaction.amount < 0,
                ~Transaction.operation_type.in_(list(known_ops)),
            )
            .group_by(Transaction.operation_type)
            .order_by(func.sum(func.abs(Transaction.amount)).desc())
            .limit(20)
        )).all()
        unmapped_total = sum(float(r.amt) for r in unmapped_rows)
        unmapped_summary = ", ".join(f"{r.op} ({float(r.amt):,.0f}₽)" for r in unmapped_rows[:5])

        total_exp = total_buckets + unmapped_total
        # пересчитываем pct по новому total
        for e in expense_breakdown:
            e.pct = round(e.amount / total_exp * 100, 1) if total_exp else 0
        expense_breakdown.sort(key=lambda x: x.amount, reverse=True)
        if unmapped_total > 0:
            expense_breakdown.append(ExpenseSegment(
                category=(f"Не опознано: {unmapped_summary}"
                          if unmapped_summary else "Не опознано"),
                amount=round(unmapped_total, 2),
                pct=round(unmapped_total / total_exp * 100, 1) if total_exp else 0,
                op_type_filter=None,
            ))
    else:
        expense_breakdown = []
    ...
    missing_n = 0
    if accs:
        # «Без себестоимости» = cost_price пуст/заглушка И активный товар.
        # Юзер: «архивные не продаются — их себест не нужна».
        # History.confidence='missing' НЕ учитываем: при bulk-правке cost_price
        # запись в product_cost_history не обновляется, и старое 'missing'
        # остаётся как аудит → давало ложные 15 при реально 1 пустом.
        # Доверяем текущему Product.cost_price.
        missing_n = int((await db.execute(
            ...
            .where(
                ...
                (Product.cost_price.is_(None)) | (Product.cost_price <= 100),
            )
        )).scalar() or 0)
