...
export function PlanVsFact() {
  ...
  const { data: planList = [] } = useQuery<PlanListItem[]>({
    ...
    queryFn: async () => (await api.get('/api/plan/sales')).data,
    enabled: tab === 'fact',
  })
  ...
  const { data: fact, isLoading: factLoading } = useQuery<FactResp>({
    ...
    queryFn: async () => (await api.get(`/api/plan/sales/${activePlanId}/fact`)).data,
  ...
  const forecastMut = useMutation({
    mutationFn: async () =>
      (await api.post('/api/plan/forecast', {
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      const { data: plan } = await api.post('/api/plan/sales', {
        cabinet_ids: selectedCabinets,
        metric,
      ...
      await api.post(`/api/plan/sales/${plan.id}/items`, { items })
      await api.post(`/api/plan/sales/${plan.id}/distribute`)
